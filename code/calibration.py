...
Updated for NEMA 11 Motors + A4988 Drivers with pigpio
...
7. Proper ENABLE pin control (motors disabled when idle)
...
CALIBRATION_SPEED = 600     # Calibration moves
PRECISION_SPEED = 200       # Slow homing approach
...
def setup():
    ...
    # Motor outputs (A4988 STEP/DIR/ENABLE)
    for pin in [MOTOR_A_STEP_PIN, MOTOR_A_DIR_PIN, MOTOR_B_STEP_PIN, MOTOR_B_DIR_PIN, MOTOR_ENABLE_PIN]:
...
# ==========================
SERVO_RELEASE_PULSE = 1500  # Raised position (microseconds)
SERVO_ENGAGE_PULSE = 500    # Lowered position (microseconds)

def disengage_magnet():
    """Raise the magnet servo to disengage from pieces (for homing)."""
    print("  Disengaging magnet (raising servo)...")
    pi.set_servo_pulsewidth(SERVO_MAGNET_PIN, SERVO_RELEASE_PULSE)
    time.sleep(0.5)
    pi.set_servo_pulsewidth(SERVO_MAGNET_PIN, 0)

def engage_magnet():
    """Lower the magnet servo to engage with pieces."""
    print("  Engaging magnet (lowering servo)...")
    pi.set_servo_pulsewidth(SERVO_MAGNET_PIN, SERVO_ENGAGE_PULSE)
    time.sleep(0.5)
    pi.set_servo_pulsewidth(SERVO_MAGNET_PIN, 0)
...
# ==========================
def read_x_limit():
    return pi.read(LIMIT_X_PIN) == 1

def read_y_limit():
    return pi.read(LIMIT_Y_PIN) == 1

def read_clock():
    return pi.read(LIMIT_CLOCK_PIN) == 1
...
# ==========================
def calculate_speed_profile(total_steps, target_speed):
...
def step_both_motors(steps_a: int, steps_b: int, speed: int = None, check_limits: bool = True):
    ...
    Motors are enabled during movement and disabled after.
    """
...
def move_x(steps: int, speed: int = None, check_limits: bool = True):
    """Move in pure X direction. Returns True if completed."""
...
def move_y(steps: int, speed: int = None, check_limits: bool = True):
    """Move in pure Y direction. Returns True if completed."""
    result = step_both_motors(steps, steps, speed, check_limits)
    if result:
        gantry.pos_y += steps
    return result
...
def _find_corner_loop(stdscr, corner_name):
    ...
    while running:
        ...
        stdscr.addstr(7, 0, "Motors: DISABLED when idle (enabled during move)")
        stdscr.addstr(9, 0, "Controls:")
        stdscr.addstr(10, 2, "↑↓←→ : Move gantry")
        stdscr.addstr(11, 2, "+/-   : Adjust step size")
        stdscr.addstr(12, 2, "q     : Confirm position")
        stdscr.refresh()
...
def find_a1_corner_semi_auto():
    ...
    estimated_distance = 7 * SQUARE_SIZE_INCHES
...
def calibrate_a1_h8():
    ...
    board_diagonal_inches = 7 * SQUARE_SIZE_INCHES
...
def show_status():
    ...
    print(f"Homed: {gantry.homed}")
    print(f"Motors: DISABLED (enabled only during movement)")
    print(f"Steps/inch: X={gantry.calibration['steps_per_inch_x']:.0f}, Y={gantry.calibration['steps_per_inch_y']:.0f}")
    print(f"A1 offset: ({gantry.calibration.get('a1_offset_x', 0)}, {gantry.calibration.get('a1_offset_y', 0)})")
...
def manual_keyboard_control():
    ...
    print("Motors are DISABLED when idle (enabled during move).")
    print("\nStarting in 2 seconds...")
    time.sleep(2)
...
def _keyboard_control_loop(stdscr):
    ...
    while running:
        ...
        stdscr.addstr(6, 0, f"Limits: X={read_x_limit()}  Y={read_y_limit()}")
        stdscr.addstr(7, 0, "Motors: DISABLED (enabled during move)")
        stdscr.addstr(9, 0, "Controls:")
        stdscr.addstr(10, 2, "↑↓←→ : Move gantry")
        stdscr.addstr(11, 2, "+/-   : Adjust speed")
        stdscr.addstr(12, 2, "q     : Quit")
        stdscr.addstr(14, 0, "Hold multiple arrows for diagonal movement")
        stdscr.refresh()
...
def main():
    ...
    try:
        while True:
            ...
            print("(pigpio + NEMA 11 + A4988 Drivers)")
            print("="*50)
